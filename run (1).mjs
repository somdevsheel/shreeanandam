...
const DELAY_BETWEEN_TEXT_AND_IMAGE = 5000;
...
async function generateAndUploadImage(title, category) {
  ...
  try {
    ...
    if (!res.ok) { log.error('Image gen HTTP ' + res.status); return null; }
    const data = await res.json();
    const part = data?.candidates?.[0]?.content?.parts?.find(p => p.inlineData?.data);
    if (!part?.inlineData?.data) { log.error('No image in Gemini response'); return null; }
    ...
    log.error('Image error: ' + e + ' — skipping article');
    return null;
  }
}
...
async function main() {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    try {
      ...
      await new Promise(r => setTimeout(r, DELAY_BETWEEN_TEXT_AND_IMAGE));
